...
async def test_transition_to_testing(client: AsyncClient, auth_headers: dict):
    ...
    await client.post("/api/v1/flags", json=make_flag_payload(key="lc-test"), headers=auth_headers)

    resp = await client.post("/api/v1/lifecycle/lc-test/transition", json={
...
async def test_transition_to_production(client: AsyncClient, auth_headers: dict):
    ...
    await client.post("/api/v1/flags", json=make_flag_payload(key="lc-prod"), headers=auth_headers)

    # development -> testing
    await client.post("/api/v1/lifecycle/lc-prod/transition", json={
        "target_stage": "testing",
    }, headers=auth_headers)

    # testing -> production
    resp = await client.post("/api/v1/lifecycle/lc-prod/transition", json={
...
async def test_invalid_transition(client: AsyncClient, auth_headers: dict):
    ...
    await client.post("/api/v1/flags", json=make_flag_payload(key="lc-invalid"), headers=auth_headers)

    # Trying to jump from development directly to stale should fail
    resp = await client.post("/api/v1/lifecycle/lc-invalid/transition", json={
        "target_stage": "stale",
    }, headers=auth_headers)
...
async def test_lifecycle_summary(client: AsyncClient, auth_headers: dict):
    ...
    await client.post("/api/v1/flags", json=make_flag_payload(key="lc-sum1"), headers=auth_headers)
    await client.post("/api/v1/flags", json=make_flag_payload(key="lc-sum2"), headers=auth_headers)
