...
async def test_add_targeting_rule(client: AsyncClient, auth_headers: dict):
    ...
    resp = await client.post("/api/v1/targeting/target-flag/rules", json={
        "priority": 1,
        "conditions": [
...
async def test_update_targeting_rules(client: AsyncClient, auth_headers: dict):
    ...
    await client.post("/api/v1/flags", json=make_flag_payload(key="target-update"), headers=auth_headers)

    resp = await client.put("/api/v1/targeting/target-update/rules", json={
        "rules": [
            {
                "priority": 1,
                "conditions": [
                    {"attribute": "country", "operator": "one_of", "value": ["US", "CA"]},
                ],
                "variation_id": "v-on",
            },
            {
...
async def test_targeting_with_segment(client: AsyncClient, auth_headers: dict):
    ...
    # Create a segment
    await client.post("/api/v1/segments", json={
        "key": "target-seg",
        "name": "Target Segment",
        "conditions": [{"attribute": "beta", "operator": "is", "value": "true"}],
    }, headers=auth_headers)

    await client.post("/api/v1/flags", json=make_flag_payload(key="target-seg-flag"), headers=auth_headers)

    resp = await client.post("/api/v1/targeting/target-seg-flag/rules", json={
        "priority": 1,
...
async def test_targeting_evaluation_with_rule(client: AsyncClient, auth_headers: dict):
    ...
    await client.put("/api/v1/flags/eval-target", json={
        "targeting_rules": [
            {
                "priority": 1,
                "conditions": [
