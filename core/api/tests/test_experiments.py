...
async def test_get_experiment(client: AsyncClient, auth_headers: dict):
    ...
    await client.post("/api/v1/flags", json=make_flag_payload(key="exp-get-flag"), headers=auth_headers)
...
async def test_start_experiment(client: AsyncClient, auth_headers: dict):
    ...
    await client.post("/api/v1/flags", json=make_flag_payload(key="exp-start-flag"), headers=auth_headers)
...
async def test_stop_experiment(client: AsyncClient, auth_headers: dict):
    ...
    await client.post("/api/v1/flags", json=make_flag_payload(key="exp-stop-flag"), headers=auth_headers)
...
async def test_add_experiment_goal(client: AsyncClient, auth_headers: dict):
    ...
    await client.post("/api/v1/flags", json=make_flag_payload(key="exp-goal-flag"), headers=auth_headers)
    ...
    if create_resp.status_code in (200, 201):
        ...
        resp = await client.post(f"/api/v1/experiments/{exp_key}/goals", json={
...
async def test_get_experiment_results(client: AsyncClient, auth_headers: dict):
    ...
    await client.post("/api/v1/flags", json=make_flag_payload(key="exp-res-flag"), headers=auth_headers)
    ...
    if create_resp.status_code in (200, 201):
        ...
        resp = await client.post(f"/api/v1/experiments/{exp_key}/results", json={
            "variation_key": "on",
            "sample_size": 100,
            "conversions": 15,
        }, headers=auth_headers)
