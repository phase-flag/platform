...
async def test_create_change_request(client: AsyncClient, auth_headers: dict):
    ...
    resp = await client.post("/api/v1/changes", json={
...
async def test_approve_change_request(client: AsyncClient, admin_headers: dict, auth_headers: dict):
    ...
    await client.post("/api/v1/flags", json=make_flag_payload(key="approve-flag"), headers=auth_headers)

    create_resp = await client.post("/api/v1/changes", json={
    ...
    if create_resp.status_code in (200, 201):
        ...
        if change_id:
            resp = await client.post(f"/api/v1/changes/{change_id}/approve", json={
...
async def test_reject_change_request(client: AsyncClient, admin_headers: dict, auth_headers: dict):
    ...
    if create_resp.status_code in (200, 201):
        change_id = create_resp.json().get("id")
        if change_id:
            resp = await client.post(f"/api/v1/changes/{change_id}/reject", json={
...
async def test_create_freeze_window(client: AsyncClient, admin_headers: dict):
    ...
    resp = await client.post("/api/v1/governance/freeze-windows", json={
        "name": "Holiday Freeze",
        "environment": "production",
...
async def test_create_service_account(client: AsyncClient, admin_headers: dict):
    ...
    resp = await client.post("/api/v1/governance/service-accounts", json={
        "name": "CI/CD Bot",
...
async def test_break_glass(client: AsyncClient, admin_headers: dict, auth_headers: dict):
    ...
    resp = await client.post("/api/v1/governance/break-glass", json={
        "flag_key": "bg-flag",
        "environment": "production",
        "action": "toggle",
