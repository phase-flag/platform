...
async def update_metrics(flag_key: str, body: MetricsUpdate, session: AsyncSession = Depends(get_session)):
    ...
    if not mig:
        raise HTTPException(status_code=404, detail="Migration not found")
    updated = await migration_service.update_metrics(
        session, mig, success_count=body.success_count, error_count=body.error_count
    )
    return _mig_to_out(updated)
