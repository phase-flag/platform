...
async def list_environments(session: AsyncSession, project_id: str, *, limit: int = 50, offset: int = 0) -> tuple[Sequence[EnvironmentDB], int]:
    base = select(EnvironmentDB).where(EnvironmentDB.project_id == project_id)
    total = (await session.execute(select(func.count()).select_from(base.subquery()))).scalar() or 0
    items = (await session.execute(base.order_by(EnvironmentDB.created_at.desc()).limit(limit).offset(offset))).scalars().all()
    return items, total
...
async def clone_environment(session: AsyncSession, source: EnvironmentDB, new_slug: str, new_name: str) -> EnvironmentDB:
