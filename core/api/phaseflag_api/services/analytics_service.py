...
async def get_flag_inventory(session: AsyncSession) -> dict[str, Any]:
    ...
    # By status
    status_stmt = (
        select(FeatureFlagDB.status, func.count().label("count"))
        .group_by(FeatureFlagDB.status)
    )
    status_rows = (await session.execute(status_stmt)).all()

    # By lifecycle stage
    lifecycle_stmt = (
        select(FeatureFlagDB.lifecycle_stage, func.count().label("count"))
    ...
    # By environment
    env_stmt = (
        select(FeatureFlagDB.environment, func.count().label("count"))
        .group_by(FeatureFlagDB.environment)
    )
    env_rows = (await session.execute(env_stmt)).all()

    # By classification
    class_stmt = (
        select(FeatureFlagDB.flag_classification, func.count().label("count"))
        .group_by(FeatureFlagDB.flag_classification)
    )
    class_rows = (await session.execute(class_stmt)).all()

    # Flags without owners
    no_owner = (await session.execute(
...
async def get_release_dashboard(session: AsyncSession) -> dict:
    ...
    stats: dict[str, Any] = {"total": len(pipelines), "completed": 0, "failed": 0, "in_progress": 0, "paused": 0}
    recent = []
    for p in pipelines:
        if p.status == "completed":
            ...
            stats["in_progress"] += 1
        recent.append({"id": p.id, "flag_key": p.flag_key, "status": p.status, "template": p.template, "created_at": p.created_at.isoformat()})
...
async def get_top_flags(session: AsyncSession, limit: int = 20) -> list[dict]:
    ...
    ).group_by(EvaluationEventDB.flag_key).order_by(func.count().desc()).limit(limit)
    result = await session.execute(stmt)
...
async def get_expired_flags(session: AsyncSession) -> list[dict]:
    ...
    result = await session.execute(stmt)
    return [{"key": f.key, "name": f.name, "expires_at": f.expires_at.isoformat(), "owner_team": f.owner_team} for f in result.scalars().all()]
...
async def get_segment_usage(session: AsyncSession) -> list[dict]:
    ...
    for seg in segments:
        ...
        flag_stmt = select(func.count()).select_from(FeatureFlagDB).where(
            FeatureFlagDB.targeting_rules.contains(seg.key)
        )
