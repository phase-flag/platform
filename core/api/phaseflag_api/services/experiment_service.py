...
from phaseflag_api.models.experiments import (
    ExperimentDB,
    ExperimentGoalDB,
    ExperimentResultDB,
)
...
def bayesian_ab_test(
    ...
    return {
        ...
        "control": {
            ...
            "total": control_total,
            "rate": round(control_conversions / max(control_total, 1), 4),
        },
        "treatment": {
            "conversions": treatment_conversions,
            "total": treatment_total,
            "rate": round(treatment_conversions / max(treatment_total, 1), 4),
        },
        "recommendation": "treatment"
        if prob_treatment_better > 0.95
        else ("control" if prob_treatment_better < 0.05 else "inconclusive"),
    }
...
def sequential_test(conversions_over_time: list[dict]) -> dict:
    ...
    for entry in conversions_over_time:
        ...
        if cumulative["control_total"] > 0 and cumulative["treatment_total"] > 0:
            ...
            z = z_test_statistic(
                cumulative["control_conv"],
                cumulative["control_total"],
