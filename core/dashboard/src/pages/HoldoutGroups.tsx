...
export default function HoldoutGroups() {
    ...
    const [editGroup, setEditGroup] = useState<HoldoutGroup | null>(null)
    const [deleteGroup, setDeleteGroup] = useState<HoldoutGroup | null>(null)
    const [expandedKeys, setExpandedKeys] = useState<Set<string>>(new Set())

    const { data: groupsData, isLoading } = useQuery({
        queryKey: ['holdout-groups'],
        queryFn: () => holdoutGroupsApi.list().then(r => r.data),
    })
    ...
    const deleteMutation = useMutation({
        mutationFn: (key: string) => holdoutGroupsApi.delete(key),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['holdout-groups'] })
            toast('success', 'Holdout group deleted')
            setDeleteGroup(null)
        },
    ...
    return (
        <div className="p-8">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Holdout Groups</h1>
                    <p className="text-gray-600 dark:text-gray-400 mt-1">Reserve a percentage of users as a control group across experiments</p>
                </div>
                <button onClick={() => setShowCreateModal(true)} className="btn btn-primary flex items-center space-x-2">
                    <Plus className="w-5 h-5" />
                    <span>New Holdout Group</span>
                </button>
            </div>
            ...
            {!isLoading && groups.length === 0 && (
                <div className="card text-center py-12">
                    <ShieldCheck className="w-12 h-12 text-gray-400 dark:text-gray-500 mx-auto mb-4" />
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">No holdout groups yet</h3>
                    <p className="text-gray-600 dark:text-gray-400 mb-4">Create a holdout group to reserve a control population across experiments.</p>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="text-left border-b border-gray-200 dark:border-gray-700">
                                ...
                                <th className="px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Holdout %</th>
                                            ...
                                            <div className="flex items-center space-x-2">
                                                <div className="w-16 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                                    <div
                                                        ...
                                                        style={{ width: `${group.holdout_percentage}%` }}
                                                    />
                                                </div>
                                                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{group.holdout_percentage}%</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
            ...
            {deleteGroup && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">Delete Holdout Group</h3>
                        <p className="text-gray-600 dark:text-gray-400 mb-6">
                            Are you sure you want to delete <strong>{deleteGroup.name}</strong>? The holdout reservation will be removed for member flags.
                        </p>
...
function HoldoutGroupModal({ group, flagKeys, onClose }: { group: HoldoutGroup | null; flagKeys: string[]; onClose: () => void }) {
    const queryClient = useQueryClient()
    const { toast } = useToast()
    ...
    const [holdoutPercentage, setHoldoutPercentage] = useState(group?.holdout_percentage ?? 10)
    const [selectedFlags, setSelectedFlags] = useState<string[]>(group?.member_flag_keys || [])
    const [flagSearch, setFlagSearch] = useState('')

    const createMutation = useMutation({
        mutationFn: (data: { key: string; name: string; description?: string; holdout_percentage: number; member_flag_keys: string[] }) =>
            holdoutGroupsApi.create(data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['holdout-groups'] })
    ...
    const updateMutation = useMutation({
        mutationFn: (data: { name?: string; description?: string; holdout_percentage?: number; member_flag_keys?: string[] }) =>
            holdoutGroupsApi.update(group!.key, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['holdout-groups'] })
            toast('success', 'Holdout group updated')
            onClose()
        },
    ...
    const handleSubmit = () => {
        if (selectedFlags.length < 1) {
            toast('error', 'At least 1 member flag is required')
            return
        }
        if (holdoutPercentage < 1 || holdoutPercentage > 50) {
            toast('error', 'Holdout percentage must be between 1 and 50')
            return
        }
        if (isEditing) {
            updateMutation.mutate({ name, description: description || undefined, holdout_percentage: holdoutPercentage, member_flag_keys: selectedFlags })
        } else {
            if (!key.trim()) { toast('error', 'Key is required'); return }
            createMutation.mutate({ key, name, description: description || undefined, holdout_percentage: holdoutPercentage, member_flag_keys: selectedFlags })
        }
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-lg w-full mx-4 p-6 max-h-[80vh] overflow-y-auto">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">
                    {isEditing ? 'Edit Holdout Group' : 'Create Holdout Group'}
                </h3>
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Holdout Group 1" className="input" />
                    </div>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Holdout Percentage ({holdoutPercentage}%)
                        </label>
                        <input
                            type="range"
                            min={1}
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Select flags to include in this holdout group</p>
                    </div>
                </div>
