export default function HowItWorks() {
  ...
  return (
    <section id="how-it-works" className="bg-pf-surface border-y border-[rgba(99,102,241,0.15)] py-20 lg:py-28">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
          {steps.map((step, i) => (
            <div key={step.number} className="relative">
              {i < steps.length - 1 && (
                <div className="hidden md:block absolute top-12 left-full w-full h-px bg-gradient-to-r from-pf-primary/40 to-transparent -translate-x-1/2" />
              )}
              <div className="text-center">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-pf-primary text-white text-xl font-heading font-light mb-6 shadow-lg shadow-pf-primary/20">
                  {step.number}
                </div>
                ...
                <p className="text-xs text-pf-primary font-medium">
                  {step.detail}
                </p>
              </div>
            </div>
